...
function guidePage(g) {
  ...
  const authorCard = `<div style="display:flex;gap:14px;align-items:flex-start;background:var(--card);border:1px solid var(--line);border-radius:14px;padding:16px 18px;margin:28px 0 8px">
    ...
    <div><div style="font-weight:700;font-size:15px">Eugene</div><div style="font-size:13px;color:var(--muted);line-height:1.55;margin-top:3px">A Singaporean dad who got fed up with cluttered, confusing finance sites &mdash; so he built a clearer one.<span id="bioMore" hidden> By day he's an entrepreneur running a few Singapore businesses; on the side he writes StockKaki's guides in plain English so the rest of us can make sense of our money. Not a licensed financial adviser &mdash; just someone who digs into the numbers so you don't have to.</span> <button type="button" onclick="var m=document.getElementById('bioMore'),h=m.hasAttribute('hidden');if(h){m.removeAttribute('hidden');this.textContent='show less';}else{m.setAttribute('hidden','');this.textContent='read more';}" style="background:none;border:0;color:var(--accent-dk);font:inherit;font-weight:600;cursor:pointer;padding:0">read more</button></div></div>
  </div>`;
