...
function emailShell(heading, lead, body, ctaText, ctaHref) {
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#FBF6EE;padding:32px 12px;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif"><tr><td align="center">
    <table role="presentation" width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;background:#FFFDF9;border:1px solid #EBE0D2;border-radius:18px;overflow:hidden">
      ...
      <tr><td style="padding:20px 32px 0"><h1 style="margin:0 0 8px;font-size:21px;color:#3A2A20;font-weight:700">${heading}</h1><p style="margin:0 0 18px;font-size:14.5px;line-height:1.6;color:#8C7A69">${lead}</p></td></tr>
      <tr><td style="padding:0 32px">${body}</td></tr>
      <tr><td align="center" style="padding:22px 32px 6px"><a href="${ctaHref}" style="display:inline-block;background:#E07A3B;color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:12px 30px;border-radius:12px">${ctaText}</a></td></tr>
      <tr><td style="padding:22px 32px 28px"><hr style="border:0;border-top:1px solid #EBE0D2;margin:0 0 14px"><p style="margin:0;font-size:12px;line-height:1.6;color:#8C7A69">You're getting this from your StockKaki alert preferences. <a href="${SITE}/account/" style="color:#B45F27">Manage your alerts</a>. Information only, not financial advice.</p></td></tr>
    </table>
...
if (TEST_TO) {
  ...
  await send(TEST_TO, 'StockKaki alerts — sample', emailShell('Ex-dividend coming up on your watchlist',
    `This is a <b style="color:#3A2A20">sample</b>. Real alerts list the stocks on <i>your</i> watchlist going ex-dividend in ~3 days.`, divTable(sample), 'View your watchlist →', `${SITE}/account/`));
...
if (ssbNow && ssbNow.code) {
  if (state.ssb && ssbNow.code !== state.ssb) {
    const body = `<table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td style="padding:12px 14px;background:#FBEADF;border-radius:12px;text-align:center"><div style="font-size:11px;color:#8C7A69;text-transform:uppercase;letter-spacing:.04em">1st-year rate</div><div style="font-size:26px;font-weight:700;color:#3A2A20;font-family:monospace">${(+ssbNow.y1).toFixed(2)}%</div></td>
      <td style="width:12px"></td>
      <td style="padding:12px 14px;background:#FBEADF;border-radius:12px;text-align:center"><div style="font-size:11px;color:#8C7A69;text-transform:uppercase;letter-spacing:.04em">10-year avg</div><div style="font-size:26px;font-weight:700;color:#B45F27;font-family:monospace">${(+ssbNow.y10).toFixed(2)}%</div></td>
    </tr></table><p style="font-size:13px;color:#8C7A69;margin:14px 0 0">Issue ${ssbNow.code}${ssbNow.applyFmt ? ` · apply by <b style="color:#3A2A20">${ssbNow.applyFmt}</b>` : ''}. From S$500, redeem any month with no penalty.</p>`;
    let sent = 0;
